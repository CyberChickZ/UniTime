"""
Evaluate a UniTime + Qwen2-VL LoRA checkpoint on GTEA.
Teacher-forced argmax over the answer span.

Usage:
    conda activate UniTime
    cd experiments/unitime/UniTime
    python eval_qwen2vl.py \
        --base_model /path/to/Qwen2-VL-2B-Instruct \
        --adapter ./checkpoints/qwen2vl_2b_gtea_run1 \
        --eval_data_path .../test.json \
        --feat_folder /path/to/features/gtea \
        --output ./results/qwen2vl_2b_gtea_run1/eval.json
"""
import os
...
def main():
    ...
    summary = {
        "experiment": f"Qwen2-VL LoRA ({args.base_model.split('/')[-1]})",
